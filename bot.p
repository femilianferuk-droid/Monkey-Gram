...
async def get_user_chats_safe(user_id: int, limit_per_account: int = 30) -> List[Dict]:
    ...
    for account in accounts:
        try:
            ...
            dialogs_count = 0
            try:
                async for dialog in client.get_dialogs(limit=limit_per_account):
                    chat = dialog.chat
                    ...
                    dialogs_count += 1
                    if dialogs_count % 10 == 0:
                        await asyncio.sleep(1)  # Задержка каждые 10 диалогов
            
            except FloodWait as e:
                logger.warning(f"FloodWait при получении чатов: {e}")
                await asyncio.sleep(e.value + 5)
            except Exception as e:
                logger.error(f"Ошибка получения чатов для {account['phone_number']}: {e}")
            
            await asyncio.sleep(3)  # Пауза между аккаунтами
            
        except Exception as e:
            logger.error(f"Ошибка обработки аккаунта {account['phone_number']}: {e}")
            continue
...
# ========== СОЗДАНИЕ ПАПКИ ==========
async def create_folder_process(callback: CallbackQuery, state: FSMContext):
...
async def create_folder_now_handler(callback: CallbackQuery, state: FSMContext):
    """Создание папки сейчас (из рассылки)"""
    await create_folder_process(callback, state)

@router.callback_query(F.data == "create_folder")
async def create_folder_handler(callback: CallbackQuery, state: FSMContext):
    """Создание папки из меню"""
    await create_folder_process(callback, state)
...
@router.callback_query(CreateFolderStates.selecting_chats, F.data == "cancel_folder")
async def cancel_folder(callback: CallbackQuery, state: FSMContext):
