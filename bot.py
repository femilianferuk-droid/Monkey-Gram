...
def get_chat_type_string(chat_type) -> str:
...
async def add_template_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        ...
        "Пришлите название для нового шаблона:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_back_button()
    )
...
async def process_template_name(message: types.Message, state: FSMContext):
    ...
    if len(template_name) > 50:
        await message.answer(
            "❌ Название шаблона слишком длинное (макс. 50 символов).\n"
            "Пришлите более короткое название:",
            reply_markup=get_back_button()
        )
        return
    ...
    await message.answer(
        f"✅ *Название принято:* `{template_name}`\n\n"
        f"Теперь пришлите текст шаблона:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_back_button()
    )
...
async def process_template_text(message: types.Message, state: FSMContext):
    ...
    if len(template_text) > 4096:
        await message.answer(
            "❌ Текст шаблона слишком длинный (макс. 4096 символов).\n"
            "Пришлите более короткий текст:",
            reply_markup=get_back_button()
        )
        return
    ...
    template_id = db.add_template(user_id, template_name, template_text)
...
async def select_template(callback: types.CallbackQuery):
    ...
    template_id, name, text = selected_template
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🗑️ Удалить шаблон", callback_data=f"delete_template_{template_id}")],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="templates_menu")]
        ])
    )
    await callback.answer()

@dp.callback_query(F.data.startswith("delete_template_"))
async def delete_template_confirm(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    template_id = int(callback.data.split("_")[2])
    
    templates = db.get_templates(user_id)
    template_name = "Неизвестно"
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
                ...
                InlineKeyboardButton(text="❌ Нет, отмена", callback_data=f"template_select_{template_id}")
            ]
        ])
    )
    await callback.answer()
...
@dp.callback_query(F.data == "view_auto_replies")
async def view_auto_replies(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    auto_replies = db.get_auto_replies(user_id)
    
    if not auto_replies:
        await callback.message.edit_text(
            "🤖 *У вас пока нет автоответов.*\n\n"
            "Добавьте автоответы для автоматической обработки сообщений.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_auto_reply_menu(user_id)
        )
    else:
        text = "🤖 *Ваши автоответы:*\n\n"
        for reply in auto_replies[:10]:
            if len(reply) == 4:  # С account_id
                reply_id, trigger, reply_text, is_active = reply
                account_id = None
            else:  # Без account_id
                reply_id, account_id, trigger, reply_text, is_active = reply
            
            status = "✅ Вкл" if is_active else "❌ Выкл"
            text += f"• *ID:* `{reply_id}`\n"
            if account_id:
                text += f"  Аккаунт: {account_id}\n"
            text += f"  Триггер: `{trigger[:30]}`\n"
            text += f"  Ответ: `{reply_text[:30]}...`\n"
...
async def add_auto_reply_start(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    accounts = db.get_user_accounts(user_id)
...
async def select_account_for_auto_reply(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    ...
    await callback.message.edit_text(
        f"🤖 *Добавление автоответа для аккаунта {account_id}*\n\n"
        f"Пришлите ключевое слово или фразу (триггер):",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_back_button()
    )
...
async def process_auto_reply_trigger(message: types.Message, state: FSMContext):
    ...
    if len(trigger) > 100:
        await message.answer(
            "❌ Триггер слишком длинный (макс. 100 символов).\n"
            "Пришлите более короткий триггер:",
            reply_markup=get_back_button()
        )
        return
    ...
    await message.answer(
        f"✅ *Триггер принят:* `{trigger}`\n\n"
        f"Теперь пришлите текст ответа:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=get_back_button()
    )
...
async def process_auto_reply_text(message: types.Message, state: FSMContext):
    ...
    if len(reply_text) > 4096:
        await message.answer(
            "❌ Текст ответа слишком длинный (макс. 4096 символов).\n"
            "Пришлите более короткий текст:",
            reply_markup=get_back_button()
        )
        return
    ...
    if reply_id:
        await message.answer(
            ...
            f"• ID: `{reply_id}`\n\n"
            f"Теперь аккаунт будет автоматически отвечать на сообщения с этим триггером.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_auto_reply_menu(user_id)
        )
    else:
        await message.answer(
            "❌ *Ошибка при создании автоответа.*",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_auto_reply_menu(user_id)
        )
...
async def toggle_check_catcher(callback: types.CallbackQuery):
    ...
    # Переключаем статус
    current_status = check_catchers[user_id].get(account_id, False)
    check_catchers[user_id][account_id] = not current_status
    
    status_text = "включен" if not current_status else "выключен"
    
    await callback.answer(f"Ловец чеков {status_text} для аккаунта {account_id}")
...
async def check_spam_block(callback: types.CallbackQuery):
    ...
    asyncio.create_task(run_spam_check_simple(user_id, account_id, callback.message))
    await callback.answer()

async def run_spam_check_simple(user_id: int, account_id: int, message: types.Message):
    """Упрощенная проверка статуса через @SpamBot - просто копируем ответ"""
    session_string = db.get_account_session(account_id, user_id)
    if not session_string:
        await message.edit_text(
            "❌ *Ошибка:* Сессия не найдена.",
    ...
    try:
        ...
        try:
            ...
            if spam_messages:
                ...
                # Берем самый последний ответ
                latest_response = spam_messages[0]['text']
                
                # Показываем ответ пользователю
                await message.edit_text(
                    f"🛡️ *Ответ от @SpamBot:*\n\n"
                    f"```\n{latest_response}\n```\n\n"
                    f"*Информация:*\n"
                    f"• Аккаунт: *{acc_info}*\n"
                    f"• ID: `{account_id}`\n\n"
                    f"*Что означает ответ:*\n"
                    f"- Если есть слова 'limited', 'restricted', 'spam' - аккаунт ограничен\n"
                    f"- Если есть 'warning', 'caution' - есть предупреждение\n"
                    f"- Если есть 'good', 'fine', 'ok' - всё в порядке\n"
                    f"- Если ответ на русском - читайте напрямую",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=get_functions_menu()
                )
            else:
                ...
                await message.edit_text(
                    f"🛡️ *@SpamBot не ответил*\n\n"
                    f"• Аккаунт: *{acc_info}*\n"
                    f"• ID: `{account_id}`\n\n"
                    f"*Возможные причины:*\n"
                    f"1. @SpamBot временно недоступен\n"
                    f"2. Аккаунт заблокирован или ограничен\n"
                    f"3. Проблемы с соединением\n\n"
                    f"*Рекомендации:*\n"
                    f"1. Проверьте вручную через @SpamBot\n"
                    f"2. Попробуйте позже\n"
                    f"3. Убедитесь, что аккаунт активен",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=get_functions_menu()
                )
            ...
            await message.edit_text(
                f"🛡️ *Ошибка проверки спам блока!*\n\n"
                f"• Аккаунт: *{acc_info}*\n"
                f"• Ошибка: {str(e)[:150]}\n\n"
