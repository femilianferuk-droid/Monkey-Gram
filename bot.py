...
temp_bookmark_data: Dict[int, Dict] = {}  # Для хранения временных данных о закладках
...
async def process_delay(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if delay < 3 or delay > 3600:  # Минимум 3 секунды для избежания FloodWait
            raise ValueError
...
async def safe_send_message(client, chat_id, text, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            await asyncio.sleep(wait_time + 1)  # +1 секунда на всякий случай
...
async def run_mass_send_to_folder(user_id: int, message: types.Message):
    ...
    try:
        ...
        # Определяем тип чатов по folder_id
        chat_types = []
        ...
        chats_in_folder = []
        try:
            async for dialog in client.get_dialogs(limit=100):  # Ограничим для производительности
                chat = dialog.chat
...
async def show_bookmarks_menu(callback: types.CallbackQuery):
    ...
    if not bookmarks:
        ...
        text = "🔖 *Ваши закладки:*\n\n"
        for bm in bookmarks[:15]:  # Покажем первые 15
            bookmark_id, account_id, chat_id, title, username, phone, acc_name = bm
            display_name = title or username or f"Chat {chat_id}"
...
async def select_account_for_bookmark(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    ...
    asyncio.create_task(load_chats_for_bookmark(user_id, account_id, session_string, callback.message))
    await callback.answer()

async def load_chats_for_bookmark(user_id: int, account_id: int, session_string: str, message: types.Message):
    """Загрузка чатов для выбора закладки"""
    client = Client(
    ...
    try:
        ...
        chats_list = []
        async for dialog in client.get_dialogs(limit=50):  # Ограничим 50 чатами
            chat = dialog.chat
...
async def goto_bookmark(callback: types.CallbackQuery):
    ...
    for bm in bookmarks:
        if bm[0] == bookmark_id:  # bookmark_id на первой позиции
            target_bookmark = bm
            break
...
async def check_spam_block(callback: types.CallbackQuery):
    ...
    asyncio.create_task(run_spam_check(user_id, account_id, callback.message))
    await callback.answer()

async def run_spam_check(user_id: int, account_id: int, message: types.Message):
    """Проверка статуса через @SpamBot"""
    session_string = db.get_account_session(account_id, user_id)
    if not session_string:
        await message.edit_text(
            "❌ *Ошибка:* Сессия не найдена.",
    ...
    try:
        ...
        await message.edit_text(
            f"🛡️ *Проверяю статус спам блока...*\n\n"
            f"Аккаунт: {acc_info}\n"
            f"Статус: Ищу @SpamBot...",
            parse_mode=ParseMode.MARKDOWN
        )
        
        # Пробуем найти @SpamBot
        try:
            # Пробуем отправить сообщение @SpamBot
            await client.send_message("spambot", "/start")
            await asyncio.sleep(2)  # Ждем ответа
            
            # Получаем историю с @SpamBot
            messages = []
            async for msg in client.get_chat_history("spambot", limit=5):
                if msg.from_user and msg.from_user.username == "spambot":
                    messages.append(msg.text or msg.caption or "")
            
            if messages:
                # Ищем информацию о спам блоке в сообщениях
                spam_info = "\n".join(messages[:3])  # Берем первые 3 сообщения
                
                # Анализируем сообщения
                status = "❓ Неизвестно"
                if any(word in spam_info.lower() for word in ["ограничен", "ограничение", "limited", "restrict"]):
                    status = "🚫 *ОГРАНИЧЕН* (Spam Block)"
                elif any(word in spam_info.lower() for word in ["нормально", "все хорошо", "good", "fine"]):
                    status = "✅ *НОРМАЛЬНО* (No Spam Block)"
                elif any(word in spam_info.lower() for word in ["предупреждение", "warning"]):
                ...
                await message.edit_text(
                    ...
                    f"• Аккаунт: *{acc_info}*\n"
                    f"• Статус: ✅ *НОРМАЛЬНО*\n"
                    f"• Причина: @SpamBot не отвечает или нет ограничений\n\n"
                    f"Аккаунт, вероятно, не имеет ограничений.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=get_functions_menu()
                )
...
# ====== ОСТАЛЬНЫЕ ФУНКЦИИ (автоответчик, шаблоны, ловец чеков) остаются без изменений ======
# Код для автоответчика, шаблонов и ловца чеков остается таким же как в предыдущей версии
# Я сохранил только основные функции для экономии места
