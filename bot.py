...
async def cmd_start(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Функции", callback_data="functions_menu")]
    ])
...
async def show_my_accounts(callback: CallbackQuery):
    ...
    for account in accounts:
        ...
        text += f"📅 Добавлен: {account['added_at']}\n"
        text += f"🟢 Активен\n\n"
        
        keyboard.row(
            InlineKeyboardButton(
                text=f"❌ Удалить {account['phone_number']}",
                callback_data=f"delete_session_{account['id']}"
            )
        )
        keyboard.row(
            InlineKeyboardButton(
                text=f"🗑️ Полностью удалить {account['phone_number']}",
                callback_data=f"full_delete_{account['id']}"
            )
        )
    
    keyboard.row(InlineKeyboardButton(text="🔙 Назад", callback_data="functions_menu"))
...
async def full_delete_account(callback: CallbackQuery):
    ...
    # Здесь можно добавить полное удаление из БД
    # В текущей реализации delete_account только деактивирует
...
async def process_mailing_message(message: Message, state: FSMContext):
    ...
    if not user_folders:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔙 Отмена", callback_data="functions_menu")]
        ])
        await message.answer(
...
async def process_folder_name(message: Message, state: FSMContext):
    ...
    # Возвращаемся к выбору папки для рассылки
    await state.set_state(MailingStates.waiting_for_folder)
...
async def select_folder_for_mailing(callback: CallbackQuery, state: FSMContext):
    ...
    asyncio.create_task(run_mailing_task(task_id, callback.from_user.id, folder_id, 
                                        message_text, messages_count, delay))
...
async def run_mailing_task(task_id: int, user_id: int, folder_id: int, 
    ...
    for i, chat in enumerate(chats[:total_chats]):
        # Выбираем сессию (можно реализовать более сложную логику)
...
async def toggle_auto_subscribe(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔙 Назад", callback_data="functions_menu")]
    ])
...
@dp.callback_query(F.data == "functions_menu")
async def functions_menu(callback: CallbackQuery):
