...
async def load_chats_for_folder(user_id: int, account_id: int, folder_id: int, session_string: str, message: types.Message):
    ...
    try:
        ...
        for dialog in dialogs:
            ...
            # Используем функцию get_chat_type_string для преобразования ChatType
            chat_type_str = get_chat_type_string(chat.type if hasattr(chat, 'type') else None)
    ...
    if db.remove_chat_from_folder(folder_chat_id, user_id):
        ...
        for chat in folder_chats:
            if chat[0] == folder_chat_id:
                ...
                folders = db.get_user_folders(user_id)
                for folder in folders:
                    if folder[0] == chat[5]:  # account_id в этом случае
                        folder_id = folder[0]
                        break
                break
        
        if folder_id:
            # Пересоздаем callback для обновления списка
            callback.data = f"view_folder_chats_{folder_id}"
