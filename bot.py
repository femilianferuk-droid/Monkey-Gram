...
class Database:
    def __init__(self, db_name="bot.db"):
        # Создаем абсолютный путь к файлу БД
    ...
    def ensure_db_directory(self):
        ...
        # Обновляем путь к БД
        self.db_name = os.path.join(data_dir, "bot.db")
    ...
    def init_db(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_folder_chats_folder_id ON folder_chats(folder_id)')
    ...
    def get_user_accounts_count(self, user_id: int) -> int:
        """Получение количества активных аккаунтов пользователя"""
        conn = self.get_connection()
        try:
            cursor = conn.cursor()
            cursor.execute(
            ...
            logger.error(f"Ошибка при получении количества аккаунтов: {e}")
            return 0
        finally:
            conn.close()
    ...
    def add_account(self, user_id: int, phone_number: str, session_name: str):
        ...
        try:
            ...
            logger.info(f"Добавлен аккаунт {phone_number} для пользователя {user_id}")
            return account_id
        except sqlite3.Error as e:
    ...
    def get_user_accounts(self, user_id: int):
        """Получение всех активных аккаунтов пользователя"""
        conn = self.get_connection()
        try:
            cursor = conn.cursor()
            cursor.execute(
            ...
            logger.error(f"Ошибка при получении аккаунтов пользователя {user_id}: {e}")
            return []
        finally:
            conn.close()
    ...
    def delete_account(self, account_id: int, user_id: int):
        ...
        try:
            ...
            if result:
                ...
                conn.commit()
                logger.info(f"Аккаунт {account_id} деактивирован для пользователя {user_id}")
                return session_name
            return None
        except sqlite3.Error as e:
    ...
    def get_account_sessions(self, user_id: int):
        """Получение имен сессий активных аккаунтов пользователя"""
        conn = self.get_connection()
        try:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def create_folder(self, user_id: int, folder_name: str):
        ...
        try:
            ...
            logger.info(f"Создана папка '{folder_name}' (ID: {folder_id}) для пользователя {user_id}")
            return folder_id
        except sqlite3.Error as e:
    ...
    def add_chat_to_folder(self, folder_id: int, chat_username: str, chat_id: int, chat_title: str):
        ...
        try:
            ...
            conn.commit()
            logger.info(f"Добавлен чат '{chat_title}' в папку {folder_id}")
        except sqlite3.Error as e:
            logger.error(f"Ошибка при добавлении чата в папку: {e}")
        finally:
            conn.close()
    ...
    def get_user_folders(self, user_id: int):
        ...
        try:
            ...
            logger.error(f"Ошибка при получении папок пользователя {user_id}: {e}")
            return []
        finally:
            conn.close()
    ...
    def get_folder_chats(self, folder_id: int):
        ...
        try:
            ...
            logger.error(f"Ошибка при получении чатов папки {folder_id}: {e}")
            return []
        finally:
            conn.close()
    ...
    def create_mailing_task(self, user_id: int, folder_id: int, message_text: str, 
        ...
        try:
            ...
            logger.info(f"Создана задача рассылки #{task_id} для пользователя {user_id}")
            return task_id
        except sqlite3.Error as e:
            logger.error(f"Ошибка при создании задачи рассылки: {e}")
            return None
        finally:
            conn.close()
    ...
    def update_mailing_progress(self, task_id: int, sent_count: int):
        ...
        try:
            ...
            logger.error(f"Ошибка при обновлении прогресса рассылки: {e}")
        finally:
            conn.close()
    
    def complete_mailing_task(self, task_id: int):
        """Завершение задачи рассылки"""
        conn = self.get_connection()
        try:
            cursor = conn.cursor()
            cursor.execute(
            ...
            conn.commit()
            logger.info(f"Задача рассылки #{task_id} завершена")
        except sqlite3.Error as e:
            logger.error(f"Ошибка при завершении задачи рассылки: {e}")
        finally:
            conn.close()
...
# Хэндлер команды /status для проверки состояния
@dp.message(Command("status"))
async def cmd_status(message: Message):
    user_id = message.from_user.id
    accounts_count = db.get_user_accounts_count(user_id)
    folders_count = len(db.get_user_folders(user_id))
    
    await message.answer(
        f"📊 Статус вашего аккаунта:\n"
        f"• Активных аккаунтов: {accounts_count}/20\n"
        f"• Папок с чатами: {folders_count}\n"
    ...
    if session_name:
        ...
        try:
            ...
            if os.path.exists(session_file):
                ...
                logger.info(f"Файл сессии удален: {session_file}")
        except Exception as e:
...
async def process_mailing_count(message: Message, state: FSMContext):
    ...
    try:
        ...
        if 1 <= count <= 500:
            ...
            await message.answer("⏱️ Введите задержку между сообщениями в секундах (от 20 до 3000):")
        else:
...
async def select_folder_for_mailing(callback: CallbackQuery, state: FSMContext):
    ...
    if task_id:
        ...
        await callback.message.edit_text("❌ Ошибка при создании задачи рассылки!")
...
async def run_mailing_task(task_id: int, user_id: int, folder_id: int, 
    ...
    for i, chat in enumerate(chats[:total_chats]):
        ...
        if success:
            ...
            logger.error(f"Не удалось отправить сообщение в чат {chat['chat_id']}")
...
async def main():
    ...
    print(f"📁 База данных: data/bot.db")
