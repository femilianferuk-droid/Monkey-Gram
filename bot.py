...
from pyrogram.types import Message, Dialog, Folder
...
def get_folders_menu(user_id: int, account_id: int):
    ...
    for folder in folders:
        ...
        included_chats = len(folder.get('included_chats', []))
        excluded_chats = len(folder.get('excluded_chats', []))
        
        keyboard.append([
            InlineKeyboardButton(
                text=f"📁 {title} ({included_chats}+/{excluded_chats}-)",
...
async def select_account_for_mass(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    ...
    await callback.message.edit_text(
        "⏳ *Загружаю папки аккаунта...*",
        parse_mode=ParseMode.MARKDOWN
    )
    
    # Загружаем папки в фоне
    asyncio.create_task(load_account_folders(user_id, account_id, session_string, callback.message))
    await callback.answer()

async def load_account_folders(user_id: int, account_id: int, session_string: str, message: types.Message):
    """Загрузка папок аккаунта"""
    client = Client(
        name=f"folders_loader_{user_id}_{account_id}",
        api_id=API_ID,
        api_hash=API_HASH,
        session_string=session_string,
        workdir="sessions"
    ...
    try:
        ...
        # Получаем папки (если API поддерживает)
        folders = []
        
        # Пробуем получить диалоги с фильтрацией
        try:
            # Для новых версий Pyrogram
        ...
        # Если не нашли папки через API, создаем стандартные
        if not folders:
            folders = [
                {'id': 0, 'title': 'Все чаты', 'included_chats': [], 'excluded_chats': []},
                {'id': 1, 'title': 'Личные сообщения', 'included_chats': [], 'excluded_chats': []},
                {'id': 2, 'title': 'Группы', 'included_chats': [], 'excluded_chats': []},
                {'id': 3, 'title': 'Каналы', 'included_chats': [], 'excluded_chats': []}
            ]
        ...
        if menu:
            await message.edit_text(
                f"✅ *Папки загружены!*\n\n"
                f"*Шаг 5/5:* Выберите папку для рассылки:\n\n"
                f"Сообщения будут отправлены во все чаты в выбранной папке.",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=menu
            )
        else:
            await message.edit_text(
                ...
                "Используйте стандартную рассылку.",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=get_functions_menu()
            )
        
    except Exception as e:
        logger.error(f"Ошибка загрузки папок: {e}")
        await message.edit_text(
            f"❌ *Ошибка загрузки папок:*\n\n`{str(e)[:200]}`",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=get_functions_menu()
        )
    finally:
...
async def select_folder_for_mass(callback: types.CallbackQuery):
    ...
    summary = (
        ...
        f"• Папка: *{folder_info.get('title', 'Неизвестно')}*\n"
...
async def run_mass_send_to_folder(user_id: int, message: types.Message):
    ...
    try:
        ...
        # В зависимости от типа папки
        if folder_id == 0:  # Все чаты
            async for dialog in client.get_dialogs():
                if dialog.chat and dialog.chat.id != user_id:
                    chats_in_folder.append(dialog.chat.id)
        elif folder_id == 1:  # Личные сообщения
            async for dialog in client.get_dialogs():
                if dialog.chat and dialog.chat.type == "private" and dialog.chat.id != user_id:
                    chats_in_folder.append(dialog.chat.id)
            ...
            async for dialog in client.get_dialogs():
                if dialog.chat and dialog.chat.type == "channel":
                    chats_in_folder.append(dialog.chat.id)
        else:
            # Для пользовательских папок
            async for dialog in client.get_dialogs():
                if (dialog.chat and dialog.chat.id != user_id and 
                    hasattr(dialog, 'folder') and dialog.folder and 
                    getattr(dialog.folder, 'id', None) == folder_id):
                    chats_in_folder.append(dialog.chat.id)
        
        total_messages = data['count'] * len(chats_in_folder)
        progress_msg = await message.edit_text(
            f"🚀 *Рассылка начата!*\n\n"
            f"Чатов в папке: *{len(chats_in_folder)}*\n"
